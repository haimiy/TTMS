...
class LecturerController extends Controller
{
    ...
    public function create()
    {
        ...
        $lecturer =  DB::table('lecturers')
            ...
            ->select('lecturers.*', 'dept_name', 'users.*')
            ->get();
        return view('lecturers.coordinator.lecturer', [
            'lecturers' => $lecturer,
            'depts' => $dept,
            'users' => $user,
    ...
    {
        $lecturer = DB::select('SELECT l.id,l.user_id,u.*,d.*,s.* FROM lecturers l 
        ...
        return view('lecturers.coordinator.lecturerProfile', [
            'lecturer' => $lecturer[0], 
            'subjects' => $subjects, 
    ...
    public function addSubjectToLecturer(Request $request){
        
        $lecturer_subject = new LecturerSubject();
        $lecturer_subject->lecturer_id = Auth::user()->id;
        $lecturer_subject->subject_id = $request['subject_id'];
        $lecturer_subject->save();
        return 'success';
    }
